...
def run_review(
    ...
    # (webhook `synchronize` would otherwise repeat every finding).
    skipped_duplicates = 0
    to_post = outcome.findings
    if post:
        existing = extract_fingerprints(gh.get_review_comment_bodies(owner, repo, number))
        if existing:
    ...
    if post:
        ...
        gh.post_review(
            ...
            repo,
            number,
            body=summary_to_markdown(summary, outcome.findings),
